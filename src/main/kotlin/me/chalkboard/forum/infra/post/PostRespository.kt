...
interface PostRepository: ReactiveCassandraRepository<PostTableModel,PostTableKey> {
    ...
    @Query("INSERT INTO forum.posts(uuid, write_day, game_id, server, title, player_name, purpose, vc_use, device, comment, created_at, user_data, tags, self_tags, play_time, delete_key, image_path) VALUES (" +
            "uuid(), :#{#model.key.writeDay}, :#{#model.key.gameId}, :#{#model.server}, :#{#model.title}, :#{#model.playerName}, :#{#model.purpose}, :#{#model.vcUse}," +
            ":#{#model.device}, :#{#model.comment}, toTimeStamp(now()), :#{#model.userData}, :#{#model.tags}, :#{#model.selfTags}, :#{#model.playTime}, :#{#model.deleteKey}, :#{#model.imagePath})")
